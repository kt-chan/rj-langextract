...
from langextract_{package_name}.schema import {provider_name}Schema"""
    ...
    provider_content = textwrap.dedent(
        f'''\
...
def create_schema(base_dir: Path, provider_name: str, package_name: str) -> None:
    ...
    schema_content = textwrap.dedent(
        ...
        class {provider_name}Schema(lx.core.schema.BaseSchema):
...
def install(base_dir: Path) -> bool:
    """Install the plugin."""
    print("\n" + "=" * 60)
...
def test(base_dir: Path) -> bool:
    """Test the plugin."""
    print("\n" + "=" * 60)
    print("Testing the plugin...")
    print("=" * 60)
...
def install_and_test(base_dir: Path) -> bool:
    install_result = install(base_dir)
    test_result = test(base_dir)
    return install_result and test_result
...
def parse_arguments():
    ...
    parser.add_argument("--test-only", action="store_true", help="testing")
